-- Migration to ensure alert emails are filtered by indicator institution for informant, reviewer, and jefatura
...
BEGIN
  IF NEW.status = 'open' AND (TG_OP = 'INSERT' OR OLD.status IS NULL OR OLD.status <> 'open') THEN
    ...
    -- Collect active informants, reviewers, and jefaturas per indicator's institution
    FOR v_assignment IN 
      SELECT 
        ii.id AS assignment_id,
