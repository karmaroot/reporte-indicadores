 ...
 *   - If reportedVal <= 0: 0%
 *   - If reportedVal <= targetVal: 100% (Cumple la meta de tiempo)
 *   - If reportedVal > targetVal: (targetVal / reportedVal) * 100 (Porcentaje efectivo de avance)
...
export function calculateIndicatorProgress(
  ...
  if (isTimeReductionUnit(unit)) {
    ...
    // Porcentaje efectivo de avance cuando se excede el tiempo: (targetVal / reportedVal) * 100
